...
export default class Select extends Component {
    ...
    render() {
        let { value, data, disable = false, name = '', label = '', filter = false, help = '', isInline = false, isObject = false, nullable = true } = this.props
        ...
        return (
            <div className={parent} id={name}>
                <label className={labels} htmlFor="text-input">{label}</label>
    ...
    setValue(property, isObject, name, event){
        ...
        if (isObject){
            this.props.onChange(property, {value: event.dataItem.value, text: event.dataItem.text})
        }
        else{
