...
module.exports = function (router) {
  ...
  router.post('/:status', async function (req, res) {
    ...
    try {
      var datum = await t_task_collection().update(update_obj, {
        where: { task_collection_id: req.body.task_collection_id }
      });
  ...
  router.put('/:id/files', async function (req, res) {
    ...
    if (!task_collection) {
      res
        .status(404)
        .json({ error: 31, message: 'Task Collection not found.' });
      return;
    }
  ...
  router.get('/download/:file_id', async function (req, res) {
    var file = await t_task_collection_file().findOne({
      where: { task_collection_file_id: req.params.file_id }
    });
    ...
    if (!!file) {
      ...
      var coll = await t_task_collection().findOne({
        where: { task_collection_id: task_collection_id }
      });
